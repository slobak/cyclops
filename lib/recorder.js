...
var Recorder = function(env, options) {
  ...
  options = options || {};
  this._options = {
    buffer: valueOrDefault(options.buffer, true),
    max_buffer_size: valueOrDefault(options.max_buffer_size, 100),
    max_buffer_time_ms: valueOrDefault(options.max_buffer_time_ms, 1000)
  };
...
Recorder.prototype.recordEvent = function(event) {
  ...
  if (me._options.buffer) {
    if ((me._buffer.length >= me._options.max_buffer_size ||
